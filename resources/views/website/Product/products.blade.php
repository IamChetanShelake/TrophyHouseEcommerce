                ...
                <div class="col-lg-10 px-2 py-4">
                    ...
                    <div id="subcategory-buttons" class="mb-4 d-flex flex-wrap gap-2 justify-content-center">
                        @foreach ($categories as $cat)
                            @foreach ($cat->subcategories as $subcat)
                                <button
                                    class="btn filter-btn subcategory-btn {{ isset($subcategoryId) && $subcategoryId == $subcat->id ? 'active' : '' }}"
    ...
    <script>
        ...
        function renderProducts() {
            products.forEach((p, index) => {
                p.style.display = (index >= (currentPage - 1) * perPage && index < currentPage * perPage) ? '' :
                    'none';
            });
        }
