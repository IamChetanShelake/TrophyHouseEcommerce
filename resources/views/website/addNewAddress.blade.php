                        ...
                        <div class="row mb-3" style="padding-top: 24px;">
                            ...
                            <div class="col">
                                <input type="text" class="form-control" name="phone" placeholder="Phone Number*"
                                    value="{{ old('phone', isset($address) ? $address->phone : '') }}" maxlength="10"
                                    required>
                                @error('phone')
                                    <div class="text-danger">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="row mb-3">
                            <div class="col">
                                <input type="email" class="form-control" name="email" placeholder="Email Address*"
                                    value="{{ old('email', isset($address) ? $address->email : '') }}" required>
                                @error('email')
                                    <div class="text-danger">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="col">
                                <input type="text" class="form-control" name="pincode" placeholder="Pincode*"
                                    value="{{ old('pincode', isset($address) ? $address->pincode : '') }}" maxlength="6"
                                    required>
                                @error('pincode')
                                    <div class="text-danger">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="row mb-3">
                            <div class="col">
                                <input type="text" class="form-control" name="country" placeholder="Country*"
                                    value="{{ old('country', isset($address) ? $address->country : '') }}" required>
                                @error('country')
                                    <div class="text-danger">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="col">
                                <input type="text" class="form-control" name="delivery_instructions"
                                    ...
                                    value="{{ old('delivery_instructions', isset($address) ? $address->delivery_instructions : '') }}">
                ...
                <div class="col-lg-4" style="padding-top: 66px;">
                    <div class="order-summary">
                        <p class="text-danger fw-bold mb-3 text-center"
                            style="font-size: 20px;font-weight:700;font-family: 'Source Sans 3', sans-serif">My Order</p>
                        <p class="fw-bold">Price Details ({{ $cart_items }} {{ $cart_items == 1 ? 'Item' : 'Items' }})
                        </p>
                        @php
                            $totalMRP = 0;
                            $totalDiscount = 0;
                            $cartItems = Auth::check()
                                ? \App\Models\cartItem::with('product.variants')->where('user_id', Auth::id())->get()
                                : collect([]); // Updated to cartItem
