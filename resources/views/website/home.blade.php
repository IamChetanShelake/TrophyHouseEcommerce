                ...
                <script>
                    ...
                    document.querySelectorAll('.category-tab').forEach(tab => {
                        tab.addEventListener('click', function() {
                            ...
                            selectedSubcategory = ""; // reset subcategory on category change
                            filterProducts();
                        });
                    });
                    ...
                    document.querySelectorAll('.subcategory-tab').forEach(tab => {
                        tab.addEventListener('click', function() {
                            selectedSubcategory = this.dataset.subcategoryId;
                            filterProducts();
                        });
                    });
                    ...
                    function filterProducts() {
                        ...
                        let [min, max] = [0, 100000];
                        if (price) {
                            [min, max] = price.split('-').map(Number);
                        }

                        fetch(
                                `/products/filter?min=${min}&max=${max}&category_id=${selectedCategory}&subcategory_id=${selectedSubcategory}`
                            )
                            .then(res => res.json())
                            .then(data => {
                                document.getElementById('products-wrapper').innerHTML = data.html;
