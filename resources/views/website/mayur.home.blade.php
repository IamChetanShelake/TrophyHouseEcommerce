...
@section('content')
    ...
    <main class="main-bg" style="background-color: white;">
                        ...
                        <ul class="nav justify-content-center" style="font-size: 14px;">
                            @foreach ($categories as $cat)
                                <li class="nav-item" style="border:none;">
                                    <a href="#"
                                        class="nav-link category-tab {{ strtolower($cat->name) === 'trophy' ? 'active' : '' }}"
                        ...
                        @foreach ($cat->subcategories as $subcat)
                            @if ($loop->index + 1 <= 9)
                                <div class="col-lg-1  col-md-1 col-sm-12 mb-3 text-center subcategory-box"
                                    data-category-id="{{ $cat->id }}">
                                    <div class="card p-2 subcategory-tab" data-subcategory-id="{{ $subcat->id }}"
                                        style="cursor: pointer;    width: 100px;border: none;">
                                        <img src="{{ asset('sub-category_images/' . $subcat->image) }}"
                                            alt="{{ $subcat->title }}" style="max-height: 100px; object-fit: contain;"
                        ...
                        <div class="row justify-content-center text-center position-relative">
                            ...
                            @foreach ($products as $prod)
                                @if ($prod->is_top_pick == 1)
                                    @if ($loop->index + 1 <= 6)
                                        <div class="col-12 d-flex justify-content-center d-sm-block col-sm-4 col-md-3 col-lg-2 mb-4 top-pick-product"
                                            ...
                                            <div class="card trophy-card text-center shadow-md">
                                                <a href="{{ route('productDetail', $prod->id) }}">
                                                    ...
                                                    <div class="card-body py-2">
                                                        ...
                                                        @else
                                                            <p class="mb-0 text-danger fw-bold">No price disclosed</p>
                                                        @endif
                                ...
                                @endif
                            @endforeach
                            <div id="new-arrival-no-products" class="no-products">No Top picks products found for this
                                subcategory.</div>
                    ...
                    <div class="trophy-card-wrapper position-relative py-5 ">
                        ...
                        <div class="row justify-content-center text-center position-relative">
                            @foreach ($products as $prod)
                                ...
                                @endif
                            @endforeach
                            <div id="top-pick-no-products" class="no-products">No Best Seler products found for this
                                subcategory.</div>
                            <div class="text-center mt-4 w-100">
                                <a href="{{ route('viewproducts') }}" class="see-more-btn">See More</a>
                            </div>
                ...
                <div class="container">
                    <div class="row align-items-center mb-4">
                        ...
                        <div class="col-lg-9 col-md-12">
                            <h2 class="text-danger fw-bold mb-2" style="font-family: 'Times New Roman', serif;">Celebrate
                                the
                                moment</h2>
                            ...
                            <p class="text-muted mb-4" style="font-family: 'Source Sans 3', sans-serif;">
                                Make this Father’s Day unforgettable with a trophy that says it all. Our limited edition
                                designs
                                are crafted to honor <br>the strength, love, and support only a dad can give. Shop now and
                                give
                            ...
                            <div class="d-flex flex-wrap gap-5 justify-content-start">
                                @foreach ($occProducts as $prod)
                                    <div>
                                        <div class="card trophy-card large-card text-center shadow-sm occasionalProduct">
                                            <div class="position-relative">
                                                <img src="{{ asset('OccasionalProduct_images/' . $prod->image) }}"
                                                    alt="Trophy" class="img-fluid"
                                                    style="max-height: 150px; width: 100%; object-fit: contain;" />
                                                <div class="trophy-hover-bar">
                                                    <i class="fas fa-heart icon-toggle wishlist-toggle {{ in_array($prod->id, $wishlist_product_ids ?? []) ? 'text-danger' : '' }}"
                                                        data-product-id="1" title="Toggle Wishlist"></i>
                                                    <form action="{{ route('cart.add') }}" method="POST">
                                                        @csrf
                                                        <input type="hidden" name="product_id"
                                                            value="{{ $prod->id }}">
                                ...
                                <div>
                                    <img src="{{ asset('website/assets/images/homePage/carousal-right.png') }}"
                                        alt="images" style="height: 35px;position: relative;left: 795px;bottom: 121px;">
                    ...
                    <div class="trophy-card-wrapper position-relative py-5">
                        ...
                        <div class="row justify-content-center text-center position-relative">
                            ...
                            @foreach ($products as $prod)
                                ...
                                @endif
                            @endforeach
                            <div id="best-seller-no-products" class="no-products">No Best Seller products found for this
                                subcategory.</div>
                            <div class="text-center mt-4 w-100">
                                <a href="{{ route('viewproducts') }}" class="see-more-btn">See More</a>
                            </div>
        ...
        <!--====== Start Clients List Section ======-->
        <section class="animated-headline-area pt-25" style="background-color: white; padding-bottom: 75px;">
            <div class="headline-wrap style-one text-center mb-3">
                <p
                    style="font-size: 36px; font-weight: 700; color: #e03c3c; font-family: 'Times New Roman', serif; text-transform: none;">
                    Client List
                            ...
                            <div class="row align-items-start">
                                ...
                                <div class="col-md-7 pb-0 position-relative"
                                    ...
                                    <div class="long-desc-view" style="position: relative; z-index: 1;">
                                        <p>At Trophy House, craftsmanship is more than just a process — it is the heart of
                                            everything we do. From the initial sketch to the final polish, every trophy is
                                            handled
                                            with care, precision, and a deep sense of purpose. We believe that awards should
                                            not
                                            only symbolize success but also embody excellence in their design and
                                            durability.
                                            That's
                                            why we use only the highest quality materials like solid metals, polished
                                            crystal,
                                            rich
                                            woods, and durable resins to ensure a flawless finish that lasts a lifetime. Our
                                            team of
                                            experienced artisans and designers work together, blending traditional
                                            techniques
                                            with
                                            modern technology to create timeless pieces that resonate with meaning. Every
                                            edge
                                            is
                                            smoothed, every detail refined, and every finish perfected — because we know
                                            that an
                                            award is not just a product, it’s a story. Whether it's for corporate
                                            milestones,
                                            sports
                                            victories, academic achievements, or personal recognition, we make sure your
                                            trophy
                                            is
                                ...
                                <div class="col-md-5 pb-0">
                                    @foreach ($aboutus as $item)
                                        <div class="about-card d-flex align-items-center mb-3">
                                            <div class="icon-box">
                                                <img src="{{ asset('about_images/' . $item->image) }}" alt="Trophy Icon">
                                            </div>
        ...
        {{-- <script>
            document.querySelectorAll('.subcategory-tab').forEach(tab => {
                tab.addEventListener('click', function() {
                    ...
                    // Top Picks
                    document.querySelectorAll('.top-pick-product').forEach(p => p.style.display = 'none');
                    document.querySelectorAll(`.top-pick-product[data-subcategory-id="${subcatId}"]`)
                        .forEach(p => p.style.display = 'block');

                    // Best Sellers
                    document.querySelectorAll('.best-seller-product').forEach(p => p.style.display = 'none');
                    document.querySelectorAll(`.best-seller-product[data-subcategory-id="${subcatId}"]`)
                        .forEach(p => p.style.display = 'block');

                    // New Arrivals
                    document.querySelectorAll('.new-arrival-product').forEach(p => p.style.display = 'none');
                    document.querySelectorAll(`.new-arrival-product[data-subcategory-id="${subcatId}"]`)
                        .forEach(p => p.style.display = 'block');
                });
            });

            // Show first subcategory's products on page load
            window.addEventListener('DOMContentLoaded', () => {
                const defaultSubcatId = document.querySelector('.subcategory-tab')?.dataset.subcategoryId;

                if (defaultSubcatId) {
                    document.querySelectorAll(`.top-pick-product[data-subcategory-id="${defaultSubcatId}"]`)
                        .forEach(p => p.style.display = 'block');
                    document.querySelectorAll(`.best-seller-product[data-subcategory-id="${defaultSubcatId}"]`)
                        .forEach(p => p.style.display = 'block');
                    document.querySelectorAll(`.new-arrival-product[data-subcategory-id="${defaultSubcatId}"]`)
                        .forEach(p => p.style.display = 'block');
                }
            });
        </script> --}}
