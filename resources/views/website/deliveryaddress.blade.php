...
<section class="container my-5">
    <div class="row g-4">
        ...
        <div class="col-lg-8">
            <p class="mb-3" style="font-family:'Source Sans 3', sans-serif;font-size:20px;font-weight:600;">Select Delivery Address</p>
            @if (session('success'))
            ...
            @else
                @if ($selectedAddress)
                    <p style="font-family:'Source Sans 3', sans-serif;font-size:18px;font-weight:600;">Default Address</p>
                    <div class="address-card mb-3 {{ $selectedAddress->is_default ? 'address-selected' : '' }}">
                        <label class="d-flex">
                            <input type="radio" name="address" class="custom-radio" value="{{ $selectedAddress->id }}" {{ $selectedAddress->is_default ? 'checked' : '' }} onclick="selectAddress(this, {{ $selectedAddress->id }})">
                            <div>
                                <p class="address-header mb-1">{{ $selectedAddress->name }}</p>
                                <p class="mb-1">{{ $selectedAddress->address }}, {{ $selectedAddress->city }}, {{ $selectedAddress->state }} - {{ $selectedAddress->pincode }}</p>
                                <p class="mb-1"><strong>Mobile:</strong> {{ $selectedAddress->phone }}</p>
                               
                                <form action="{{ route('address.delete', $selectedAddress->id) }}" method="POST" style="display: inline;">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn btn-sm btn-outline-danger" onclick="return confirm('Are you sure you want to delete this address?')">Remove</button>
                                </form>
                                <a href="{{ route('address.edit', $selectedAddress->id) }}" class="btn btn-sm btn-outline-secondary me-2">Edit</a>
                            </div>
                        </label>
                    </div>
                @endif
                ...
                @if ($addresses->where('is_default', false)->count() > 0)
                    <p style="font-family:'Source Sans 3', sans-serif;font-size:18px;font-weight:600;">Other Addresses</p>
                    @foreach ($addresses->where('is_default', false) as $address)
                        <div class="address-card mb-3 {{ $address->id == ($selectedAddress->id ?? 0) ? 'address-selected' : '' }}">
                            <label class="d-flex">
                                <input type="radio" name="address" class="custom-radio" value="{{ $address->id }}" {{ $address->id == ($selectedAddress->id ?? 0) ? 'checked' : '' }} onclick="selectAddress(this, {{ $address->id }})">
                                <div>
                                    <p class="address-header mb-1">{{ $address->name }}</p>
                                    <p class="mb-1">{{ $address->address }}, {{ $address->city }}, {{ $address->state }} - {{ $address->pincode }}</p>
                                    ...
                                    <a href="{{ route('address.edit', $address->id) }}" class="btn btn-sm btn-outline-secondary me-2">Edit</a>
                                </div>
                            </label>
                        </div>
                    @endforeach
        ...
        <div class="col-lg-4" style="padding-top: 54px;">
            <div class="price-box">
                <p class="mb-3" style="font-family:'Source Sans 3', sans-serif;font-size:20px;font-weight:600;">DELIVERY ESTIMATES</p>
                ...
                @foreach ($cartItems as $item)
                    ...
                    <div class="d-flex align-items-center mb-2">
                        <img src="{{ asset('product_images/' . $item->product->image) }}" class="icon-trophy" alt="product">
                        @php
                        ...
                        <small>
                            {{ $item->product->title }}
                            @if($variant && $variant->size) ({{ $variant->size }} - inch) @endif
                            (x{{ $item->quantity }}) - ₹{{ number_format($itemTotal, 2) }}<br>
                            <!--Delivery between 16 June - 18 June-->
                        </small>
                    </div>
                @endforeach
                    Delivery between {{ $startDate }} - {{$endDate}}
