            ...
            <tr>
                ...
                <td>
                    @if ($req->status === 'pending' && $req->designer_id === null)
                        <form method="POST" action="{{ route('designer.accept', $req->id) }}">
                            @csrf
                            <button class="btn btn-success btn-sm">Accept</button>
                        </form>
                        <form method="POST" action="{{ route('designer.reject', $req->id) }}" class="d-inline">
                            @csrf
                            <button class="btn btn-danger btn-sm">Reject</button>
                        </form>
                    @elseif ($req->status === 'accepted' && $req->designer_id === Auth::id())
                        <a href="{{ route('designer.workspace', $req->id) }}" class="btn btn-primary btn-sm">Go to Workspace</a>
                    @endif
                </td>
            </tr>
        @endforeach
